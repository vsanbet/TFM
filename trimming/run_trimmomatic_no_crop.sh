...
if [[ ! -f "${SCRIPT_DIR}/trimmomatic_array_no_crop.sh" ]]; then
  echo "Error: trimmomatic_array_no_crop.sh not found in ${SCRIPT_DIR}"
  exit 1
fi
...
sbatch --account=nn8029k --time=8:00:00 --mem=32G --cpus-per-task=8 -N 1 \
  --array=2-${LAST_LINE} \
  "${SCRIPT_DIR}/trimmomatic_array_no_crop.sh" "$SAMPLES_TSV" "$OUTDIR"
